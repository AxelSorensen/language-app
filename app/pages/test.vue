  ...
  if (!translateModeComp.state.value.translateMode) {
    ...
    if (activeElement && activeElement.tagName === "INPUT") {
      ...
      if (idx !== -1 && idx !== undefined) {
        ...
        // If in translate mode and current word is empty, delete it
        if (words.value[idx].text.trim() === "") {
          words.value.splice(idx, 1);
          // Clear previousSelection since the word is deleted
  ...
  if (activeElement && activeElement.tagName === "INPUT") {
    if (key === " ") {
      ...
      if (idx !== -1 && idx !== undefined) {
        ...
        return (cursorPos === 0 && idx > 0) || isVirtual;
      } else {
        // Normal backspace for other inputs (e.g., translateInput)
        handleBackspace(activeElement, -1);
        return true;
      }
